...
def calculate_substrate_height(row_data, bin_size=0.5):
    ...
    Returns:
        float: The calculated substrate height. Raises error if calculation fails.

    Notes:
        If the provided bin size results in fewer than 2 bins, the bin size is repeatedly reduced by a factor of 8
        until a valid histogram can be created. If a bin size smaller than 1e-6 is reached, an error is raised.
    """
    ...
    if bins.size < 4:
        if row_min == row_max:
            raise ValueError("All values in the row are identical. Data likely invalid.")
        else:
            while bins.size < 4:
                bin_size /= 16.0
                if bin_size < 1e-6:
                    raise ValueError("Unable to determine suitable bin size for substrate height calculation.")
                bins = np.arange(row_min, row_max + bin_size, bin_size)
